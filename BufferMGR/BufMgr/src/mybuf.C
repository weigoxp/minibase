...
BufMgr::BufMgr (int numbuf, Replacer *replacer) {
  // put your code here
    ...
    Page* bufPool =  new Page[numBuffers];
    

    descriptors* desc =  new descriptors[numBuffers];

    // implement hash table here.
    // an array of pointers to lists of <page number, frame number> pairs. 
    // The array is called the directory and each list of pairs is called a bucket. 
    list<pair> bucket;
    bucket* hashtable = new bucket[NUMBUF];
}
...
Status BufMgr::pinPage(PageId PageId_in_a_DB, Page*& page, int emptyPage) {
    ...
    hashtable[hashfunction(PageId_in_a_DB)]
