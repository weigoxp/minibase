...
Status BTreeFile::insert(const void *key, const RID rid) {
    ...
    if(headerPage->pageId ==-1) // there is no root; need to create the root
	{
	PageId  rootId;
	//Page *pg;
	BTLeafPage *root;
		MINIBASE_BM->newPage(rootId,(Page*&)root,1);
	...
	else
	{
		...
		else if(entry!=NULL && splitPageId!=headerPage->pageId)
		{
			...
			BTIndexPage *root;
			RID rid;
			MINIBASE_BM->pinPage(headerPage->pageId, (Page*&)root, 0,fileName.c_str());
			//BTIndexPage *root = (BTIndexPage *)pg;
			status = root->insertKey(&entry->key.intkey,headerPage->keyType,entry->data.pageNo,rid);
...
IndexFileScan *BTreeFile::new_scan(const void *lo_key, const void *hi_key) {
  // put your code here
	...
	scan->lowVal = lo_key;
	scan->highVal = hi_key; 
	scan->keySize = headerPage->keySize;
	scan->type = headerPage->keyType;
	// We will do a traditional and the highly efficient 
	// btree traversal here with the log(B)_F
...
///////////////////////////private function
//create data record to insert into leaf page
...
///////////////////////////private function
// create data entry to insert into index page
