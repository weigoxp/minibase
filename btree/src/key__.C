...
{
  // put your code here
  // for index page, memory allocation is enough with the
  // size of a PageId in a page
	if(ndtype == INDEX)
	{
	...
	// for leaf page, memory allocation is enough with the
  // size of a RID as we have record pointers in the leaf node
	else if(ndtype == LEAF)
	{
		if(key_type == attrInteger)
		{
