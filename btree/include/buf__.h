...
class PageReplacer {
private:
    stack<PageId> MruStack; // MRU uses a stack to evict most recently used
    queue<PageId> LruQueue; // LRU uses a queue to evict least recently used
...
public:
    ...
    Status unpinPage(PageId globalPageId_in_a_DB, int dirty, int hate);
        ...
        // if pin_count>0, decrement it and if it becomes zero,
